...
/** Track a generated Blueprint report being shown (preview or unlocked), by funnel entry source. */
export function trackBlueprintView(source: string): void {
  const payload = JSON.stringify({
    page: window.location.pathname,
    type: "blueprint_view",
    hardware: source,
...
/** Track a click on the $19 Blueprint checkout button. */
export function trackBlueprintPurchaseClick(source: string): void {
  const payload = JSON.stringify({
    page: window.location.pathname,
    type: "blueprint_purchase_click",
    vendor: "lemonsqueezy",
    hardware: source,
  });
